# -*- coding: utf-8 -*-
"""
From https://gist.github.com/SixQuant/4ebfb840f8a9703f609e1ce472258376
"""
...
def plt_figure_by_pixel(num=None,  # autoincrement if None, else int from 1-N
...
def plt_set_margin_by_pixel(top=None, bottom=None, left=None, right=8,
                            fontsize=None, dpi=None):
    """
...
def plot_table(data,
    ...
    row_height = plt_get_text_height_pixel(fontsize) \
        * (1 if r > 2 else 2) + 4 + 4
    ...
    if title is not None and title != '':
        title, color = (title, None) if type(title) is str else (title[0],
                                                                 title[1])
        plt.title(title, ax=ax, color=color, fontsize=fontsize + 2)
        top = plt_get_text_height_pixel(fontsize + 2) + 4 + 4

    table = ax.table(cellText=data[1:], colWidths=col_widths,
                     bbox=[0, 0, 1, 1], colLabels=data[0], cellLoc='center',
                     **kwargs)
...
def set_table_cell_default_style(table, header_colors=None, no_header=False,
                                 row_colors=None, edge_color='w'):
